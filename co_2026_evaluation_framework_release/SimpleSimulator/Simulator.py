...
def simulate(bin_lines):
    ...
    for _ in range(200000):
        ...
        if opcode== 0b0110011:
            ...
            f3=(instr>>12)&0x7; rs1=(instr>>15)&0x1F; rs2=(instr>>20)&0x1F
            imm=sign_ext(((instr>>31)&1)<<12|((instr>>7)&1)<<11|((instr>>25)&0x3F)<<5|((instr>>8)&0xF)<<1,13)
            v1,v2=to_int32(regs[rs1]),to_int32(regs[rs2]); u1,u2=regs[rs1],regs[rs2]
            br=(f3==0 and v1==v2)or(f3==1 and v1!=v2)or(f3==4 and v1<v2)or(f3==5 and v1>=v2)or(f3==6 and u1<u2)or(f3==7 and u1>=u2)
            ...
            rd=(instr>>7)&0x1F
            if rd: regs[rd]=u32(PC+(instr&0xFFFFF000))

        elif opcode == 0b1101111:   # jal
            rd=(instr>>7)&0x1F
            imm=sign_ext(((instr>>31)&1)<<20|((instr>>12)&0xFF)<<12|((instr>>20)&1)<<11|((instr>>21)&0x3FF)<<1,21)
            if rd: regs[rd]=u32(PC+4)
            next_PC=u32(PC+imm)
    ...
    mem_lines=['0x{:08X}:{}'.format(DATA_START+i*4,to_bin32(data_mem.get(DATA_START+i*4,0))) for i in range(DATA_WORDS)]
    return trace_lines, mem_lines
